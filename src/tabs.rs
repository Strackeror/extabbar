...
use bindings::Windows::Win32::System::OleAutomation::VARIANT;
use bindings::Windows::Win32::System::WindowsProgramming::IWebBrowserApp;
...
struct Obj {
    ...
    web_browser_interface: IWebBrowserApp,
}
...
impl TabBar {
    pub fn new(parent: HWND, web_browser_interface: IWebBrowserApp) -> TabBar {
        let handle = unsafe {
            CreateWindowExW(
                WINDOW_EX_STYLE(0),
                "SysTabControl32",
    ...
    pub fn navigated(&self, path: String) -> Result<()> {
        if let Some(index) = self.0.get_selected_tab_index() {
            self.0.get_tab(index)?.path = path.clone();
...
impl TabBarRef {
    fn duplicate_tab(&self) -> Result<()> {
        ...
        self.add_tab(current_tab.path, current_index)
    }

    fn add_tab(&self, path: String, idx: usize) -> Result<()> {
        let key: usize;
        {
            ...
            key = *key_counter;
            *key_counter += 1;
            tabs.insert(key, Tab { path: path.clone() });
        }
    ...
    fn tab_switched(&self) -> Result<()> {
        ...
        let browser = self.0.borrow().web_browser_interface.clone();
